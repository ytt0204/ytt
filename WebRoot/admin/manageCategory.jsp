...
<tr align="center" bgcolor="#FAFAF1" height="22">
	...
	<c:forEach items="${sessionScope.categorylist}" var="c" >
		...
		<td><input name="cid" type="checkbox" id="id" value="${c.cid }" class="np"></td>
		<td>${c.cid }</td>
		<td>${c.cname}</td>
		
		<td><a href="Category.action?oper=findbyid&id=${c.cid }">修改</a> | 
		<a href="javascript:delbyid('Category.action?oper=delbyid&id=${c.cid }')">删除</a></td>
	</tr>
	</c:forEach>
...
<tr align="right" bgcolor="#EEF4EA">
	...
	<a href="Category.action?oper=showall&page=1" class="coolbg">首页</a>&nbsp;
	<c:choose >
		<c:when test="${sessionScope.pageBean.currentpage>1}">
		<a href="PCategory.action?oper=showall&page=${sessionScope.pageBean.currentpage-1 }" class="coolbg">上一页</a>&nbsp;
		</c:when>
		<c:otherwise>
	...
	<c:forEach var="index" begin="1" end="${sessionScope.pageBean.pagecount}">
		<a href="Category.action?oper=showall&page=${index }" class="coolbg">${index }</a>&nbsp;
	</c:forEach>
	<c:choose >
		<c:when test="${sessionScope.pageBean.currentpage<sessionScope.pageBean.pagecount}">
		<a href="Category.action?oper=showall&page=${sessionScope.pageBean.currentpage+1 }" class="coolbg">下一页</a>&nbsp;
		</c:when>
		<c:otherwise>
	...
	<a href="Category.action?oper=showall&page=${sessionScope.pageBean.pagecount }" class="coolbg">末页</a>&nbsp;
